...
#nice stats about the categorical columns
def get_cat_num_col_stats(PATH='../datasets', graph='no',return_dict_of_cols='n'):
    ...
    if graph == 'yes':
        ...
        plt.savefig('Graph1_total_columns.png')
        plt.show()
        ...
        plt.savefig("Graph2_count_type.png")
        plt.show()
...
#
def preprocess_dataset(dataframe,label_col=None):
    #check if label column was passed
    if label_col == None:
        #if it was not passed we keep the Y_true as the class
        Y_true = dataframe[label_col].values
        dataframe = dataframe.drop('class',axis=1)
    else: 
        Y_true = dataframe['class'].values
...
def preprocess_dataset(dataframe,label_col=[],ohe='y'):
    ...
    df_nums = dataframe.select_dtypes(include='number')
    if len(df_nums)>0:
        distributions = preprocess_numerical(df_nums)
    ...
    def preprocess_numerical(df_nums):
